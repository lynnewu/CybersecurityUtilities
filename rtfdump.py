...
__author__ = 'Didier Stevens, Lynne Whitehorn, ChatGPT5'
__version__ = '1.0.0'
__date__ = '2025/08/21'
...
History:
  ...
  2016/01/23: added support for -c :-number
  ...
  2025/08/20: forked from the original using ChatGPT5 to add code to do malware analysis of the RTF file.  Not even vaguely tested.
...
SUSPICIOUS_PATTERNS_RTF = [
    ('RTF_OBJECT', rb'\\(?:obj(?:ect|data|class)|pict)\b'),
    ('DDE', rb'\bDDEAUTO\b|\bDDE\b'),
    ('HYPERLINK', rb'\bHYPERLINK\b'),
    ('INCLUDEPICTURE', rb'\bINCLUDEPICTURE\b'),
    ('EQUATION', rb'\b(?:Equation\.3|EQNEDT32)\b'),
    ('PACKAGE', rb'\bPackage\b'),
    ('URL', rb'https?://[^\s\\}]+'),
    ('MS_PROTOCOL', rb'\bms-msdt:'),
]
SUSPICIOUS_PATTERNS_DECODED = [
    ('URL', rb'https?://[^\s\\}]+'),
    ('LOLBIN', rb'\b(?:mshta|powershell|cmd\.exe|wscript|cscript|rundll32|regsvr32)\b'),
    ('MS_PROTOCOL', rb'\bms-msdt:'),
]
...
def _read_ads_zone_identifier(filepath):
    try:
        ...
        try:
            ...
            with open(ads_path, 'r', encoding='utf-16') as f:
                data = f.read()
        info = {}
        m = re.search(r'ZoneId\s*=\s*(\d+)', data)
        if m: info['ZoneId'] = int(m.group(1))
        for key in ('ReferrerUrl', 'HostUrl', 'URL'):
            m = re.search(r'%s\s*=\s*(.+)' % key, data)
            if m: info[key] = m.group(1).strip()
        return info if info else None
    except Exception:
        return None

def _decode_hex_local(hexstring, shift=False):
    # Local variant of HexDecode so we don't mutate options
    res = b''
    for entry in hexstring:
        if isinstance(entry, str):
            s = entry
            if len(s) % 2 == 1:
                s = ('0' + s) if shift else (s + '0')
            elif shift:
                s = '0' + s + '0'
...
def SafeScan(rtfdata, dAnalysis, remainder_bytes, input_filename=None):
    out = {
        'counts': {},
        'hits': [],               # list of {type, where, offset, sample}
        'urls': [],
        'ole_objects': [],        # list of {class, size, hash_type, hash, magic}
        'appended_bytes': len(remainder_bytes) if remainder_bytes else 0,
        'motw': None,
        'risk': {'level': 'low', 'score': 0, 'reasons': []},
    }

    # 1) Scan raw RTF bytes for control words & obvious markers
    for name, pat in SUSPICIOUS_PATTERNS_RTF:
        hits = list(re.finditer(pat, rtfdata, re.I))
        if hits:
            out['counts'][name] = len(hits)
            for m in hits[:20]:  # cap displayed hits
                start = max(0, m.start() - 24)
                end = min(len(rtfdata), m.end() + 24)
                sample = rtfdata[start:end]
                try: sample_txt = sample.decode('latin-1', 'replace')
                except: sample_txt = repr(sample)
                out['hits'].append({'type': name, 'where': 'rtf', 'offset': m.start(), 'sample': sample_txt})

    # Extract URLs from RTF scan for convenience
    for h in [h for h in out['hits'] if h['type'] == 'URL']:
        out['urls'].append(h['sample'])
    # 2) Scan hex-decoded data from each group for URLs/lolbins/protocols
    decoded_urls = []
    for a in dAnalysis.values():
        if a.hexstring is None:
            continue
        for shift in (False, True):
            dec = _decode_hex_local(a.hexstring, shift=shift)
            if not dec:
                continue
            for name, pat in SUSPICIOUS_PATTERNS_DECODED:
                for m in re.finditer(pat, dec, re.I):
                    start = max(0, m.start() - 24)
                    end = min(len(dec), m.end() + 24)
                    sample = dec[start:end]
                    try: sample_txt = sample.decode('latin-1', 'replace')
                    except: sample_txt = repr(sample)
                    out['hits'].append({'type': name, 'where': 'decoded', 'offset': m.start(), 'sample': sample_txt})
                    if name == 'URL':
                        decoded_urls.append(sample_txt)
    if decoded_urls:
        out['urls'].extend(decoded_urls)

    # 3) OLE objects summary
    for a in dAnalysis.values():
        if a.oleInfo != []:
            cls = (a.oleInfo[0] if isinstance(a.oleInfo[0], bytes) else str(a.oleInfo[0]))
            try:
                cls_txt = cls.decode('latin-1') if isinstance(cls, (bytes, bytearray)) else str(cls)
            except:
                cls_txt = repr(cls)
            out['ole_objects'].append({
                'class': cls_txt,
                'size': a.oleInfo[2],
                'hash_type': a.oleInfo[3][1],
                'hash': a.oleInfo[3][0],
                'magic': (a.oleInfo[4].decode() if isinstance(a.oleInfo[4], (bytes, bytearray)) else str(a.oleInfo[4])),
            })

    # 4) MOTW (Windows only)
    if input_filename and sys.platform == 'win32' and os.path.isfile(input_filename):
        out['motw'] = _read_ads_zone_identifier(input_filename)

    # 5) Risk scoring
    score = 0; reasons = []
    def add(n, why):
        nonlocal score; score += n; reasons.append(why)

    if out['counts'].get('DDE'): add(3, 'DDE field')
    if out['counts'].get('EQUATION'): add(3, 'Equation Editor reference')
    if any('ms-msdt' in h['sample'].lower() for h in out['hits']): add(3, 'ms-msdt protocol')
    if any(o for o in out['ole_objects'] if 'Package' in o['class'] or 'Equation' in o['class']): add(3, 'OLE Package/Equation object')
    if any(h for h in out['hits'] if h['type'] == 'LOLBIN'): add(2, 'LOLBin reference')
    if out['counts'].get('RTF_OBJECT'): add(1, 'RTF object/pict')
    if out['counts'].get('HYPERLINK') or out['counts'].get('INCLUDEPICTURE'): add(1, 'Hyperlink/IncludePicture')
    if out['urls']: add(1, 'Has URLs')
    if out['appended_bytes'] > 0: add(1, 'Bytes appended after last group')

    level = 'low'
    if score >= 4: level = 'high'
    elif score >= 2: level = 'medium'
    out['risk'] = {'level': level, 'score': score, 'reasons': reasons}
    ...
    out['urls'] = _uniq_preserve(urls)[:30]

    return out

def PrintSafeScan(report):
    print('=== SafeScan summary ===')
    print('Risk: {level} (score {score})'.format(**report['risk']))
    if report['risk']['reasons']:
        print('Reasons: ' + ', '.join(report['risk']['reasons']))
    if report['motw'] is not None:
        z = report['motw'].get('ZoneId', '?')
        print(f'Mark-of-the-Web: present (ZoneId={z})')
    else:
        if sys.platform == 'win32':
            print('Mark-of-the-Web: not present or unreadable')

    if report['counts']:
        print('\nCounts:')
        for k in sorted(report['counts'].keys()):
            print(f'  {k:14s} : {report["counts"][k]}')

    if report['ole_objects']:
        print('\nOLE objects:')
        for o in report['ole_objects'][:10]:
            print(f'  Class={o["class"]!s} Size={o["size"]} {o["hash_type"]}={o["hash"]} magic={o["magic"]}')

    if report['urls']:
        print('\nURLs (first {n}):'.format(n=min(len(report["urls"]),10)))
        for u in report['urls'][:10]:
            print('  ' + u)

    shown = 0
    for h in report['hits']:
        if shown >= 12: break
        print(f'\n[{h["type"]}] where={h["where"]} offset=0x{h["offset"]:x}')
        print('  ' + h['sample'].replace('\r', ' ').replace('\n', ' ')[:160])
        shown += 1

# --- End SafeScan additions ---------------------------------------------------
...
def RTFSub(oBytesIO, prefix, rules, options):
    ...
    # Run SafeScan after analysis is built
    if getattr(options, 'safescan', False):
        report = SafeScan(rtfdata, dAnalysis, remainder, getattr(options, '_input_filename', None))
        if options.jsonoutput:
            print(json.dumps({'safescan': report}))
        else:
            PrintSafeScan(report)
        # (Do not return here to preserve normal behavior; uncomment to make -K standalone)
        # return returnCode
...
def RTFDump(filename, options):
    ...
    # record input name for safescan/MOTW
    try:
        options._input_filename = filename if filename not in ('', None) and not filename.lower().endswith('.zip') else None
    except Exception:
        pass
...
def Main():
    ...
    oParser.add_option('-K', '--safescan', action='store_true', default=False,
                       help='Quick triage for suspicious constructs (DDE, OLE/Package/Equation, URLs, LOLBINs); prints a risk summary (JSON if used with --jsonoutput)')
